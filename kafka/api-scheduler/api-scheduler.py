...
def schedule_new_city(partition, list_city_to_process, processing):
    ...
    queue.using(partition=partition, key=None, timeout=TIMEOUT).enqueue(
        helper.extract_from_api, city, list_apis[city])
    processing[city] = 0
    producer.flush()
    print(f"adding {city} to processing\n")


def retry(partition, processing, city):
    queue.using(partition=partition, key=None, timeout=TIMEOUT).enqueue(
        helper.extract_from_api, city, list_apis[city])
    processing[city] += 1
    print(
        f"Retrying {city} for the {processing[city]}th time in partition {partition} \n")


def print_status(list_city_to_process, processing):
...
def job():
    ...
    print_status(list_city_to_process, processing)
    # for the rest
    print("\nScheduling the rest\n")
    ...
    for completed in consumer:
        ...
        print(
            f"city {city} is {'finished' if {finished} else 'not finished'} by partition {partition}")
        ...
        if finished == True and city in processing:
            ...
            if processing[city] < MAX_RETRIES:
                retry(partition, processing, city)
            else:
                print(f"removing {city} from processing due to max retries \n")
                processing.pop(city)
                if len(list_city_to_process) > 0:
                    schedule_new_city(
                        partition, list_city_to_process, processing)
                elif len(processing) == 0:  # No more cities to process
                    break
